...
class gdate
{
    ...
    public function miladi_to_shamsi($miladi)
    {
        $miladi_list = explode("-", $miladi);
        $l = gregorian_to_jalali($miladi_list[0], $miladi_list[1], $miladi_list[2]);
        return implode("/", $l);
    }

    public function shamsi_to_miladi($shamsi, $splitter = '/')
    {
        $shamsi_list = explode($splitter, $shamsi);
        $l = jalali_to_gregorian($shamsi_list[0], $shamsi_list[1], $shamsi_list[2]);
        return implode("-", $l);
    }
    ...
    public function miladi_to_shamsi_datetime($miladi)
    {
        $miladi_list = explode("-", $miladi);
        $time = explode(" ", $miladi_list[2])[1];
        $l = gregorian_to_jalali($miladi_list[0], $miladi_list[1], $miladi_list[2]);
        return $time . " " . implode("/", $l);
    }

    public function get_time($datetime)
    {
        $datetime_list = explode("-", $datetime);
        $time = explode(" ", $datetime_list[2])[1];
        return $time;
    }
