                                    ...
                                    <tbody>
                                        @foreach ($users as $user)
                                            <tr>
                                                ...
                                                <td  class="d-flex"><a href="{{ route('user.editUser', ['accountId' => $account->id, 'userId' => $user->id]) }}" class="btn btn-warning  m-1">ویرایش</a>
                                                <form action="{{ route('account.users.destroy', ['accountId' => $account->id, 'userId' => $user->id]) }}" class="mt-1  m-1" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-danger">حذف کاربر</button>
                                                </form>
                                                    @if($user->user_status == 'Active')
                                                    <button type="button" name="active" class="btn btn-danger Deactive-button" data-toggle="modal" data-target="#exampleModal" id="{{ $user->id }}" value="DeActive">غیرفعال سازی کاربر</button>
                                                    @elseif ($user->user_status == 'DeActive')
                                                    <form action="{{ route('account.users.activation') }}" class="mt-1  m-1" method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                        <input type="hidden" name="id" value="{{ $user->id }}">
                                                        <button type="submit" name="active" class="btn btn-success" value="Active">فعال سازی کاربر</button>
                                                </form>
                                                    @endif
                                            </td>
                                            </tr>
